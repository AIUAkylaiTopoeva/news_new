...
class CommentInline(admin.TabularInline):#Это Django-класс, который позволяет отображать связанные объекты (в данном случае комментарии) в форме для редактирования родительской модели (в данном случае статьи).
    model = Comment
    # extra = 0
